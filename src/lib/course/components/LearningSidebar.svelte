...
{#if courseState.isSidebarOpen}
	...
	<div
		...
		class="fixed inset-0 bg-black/10 backdrop-blur-[1px] z-40 cursor-zoom-out"
...
<!-- 側邊標籤按鈕（位置與側邊欄連動） -->
<button
	class="pixel-button fixed top-1/2 -translate-y-1/2 z-[60] w-fit px-[10px] py-[16px] flex flex-col items-center gap-1.5 transition-all duration-400 !m-0 bg-[var(--bg-main)] text-[#4b3324] cursor-pointer select-none active:translate-y-[calc(-50%+2px)] shadow-[0_-3px_0_0_#4b3324,0_3px_0_0_#4b3324,-3px_0_0_0_#4b3324,3px_0_0_0_#4b3324,0_-6px_0_0_#d4a745,0_6px_0_0_#d4a745,-6px_0_0_0_#d4a745,6px_0_0_0_#d4a745,0_-9px_0_0_#4b3324,0_9px_0_0_#4b3324,-9px_0_0_0_#4b3324,9px_0_0_0_#4b3324] hover:brightness-[1.02]"
...
>
	...
	<span class="relative z-10 [writing-mode:vertical-lr] font-black tracking-[0.2em] text-lg uppercase select-none leading-none">
		{courseState.isSidebarOpen ? '關閉課文' : '查看課文'}
	</span>
	<svg xmlns="http://www.w3.org/2000/svg" class="relative z-10 h-6 w-6 transform transition-transform duration-500" class:rotate-180={courseState.isSidebarOpen} fill="none" viewBox="0 0 24 24" stroke="currentColor">
	...
	<div class="p-8 h-full overflow-y-auto custom-scrollbar">
		<ul class="space-y-4">
			{#each courseState.lessons as lesson}
				<li class="flex items-start gap-4 p-2 rounded-xl hover:bg-blue-50/50 transition-all group">
					<div class="mt-3.5 h-1.5 w-1.5 rounded-full bg-blue-500 shrink-0 opacity-40 group-hover:opacity-100 transition-opacity"></div>
					<div class="flex-1">
						<h4 class="font-bold text-gray-900 mb-1 leading-tight group-hover:text-blue-600 transition-colors">
							{lesson.fullId}
						</h4>
