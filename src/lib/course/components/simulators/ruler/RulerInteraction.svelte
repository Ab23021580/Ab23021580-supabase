...
{#if type === 'STATIC'}
	...
	>
		<circle cx={offsetX + 19.5 * pxPerCm} cy={65} r="30" fill="#DABEA7" fill-opacity="0.3" class="group-hover:fill-opacity-100 transition-all" />
		<circle cx={offsetX + 19.5 * pxPerCm} cy={65} r="18" stroke="#ef4444" stroke-width="2" fill="none" stroke-dasharray="4 2" />
		<text x={offsetX + 19.5 * pxPerCm} y="30" text-anchor="middle" font-size="12" font-weight="bold" fill="#ef4444" class="animate-bounce">點擊此處</text>
	</g>
{/if}
...
{#if type && (type.includes('DRAG') || type === 'PRECISION')}
	...
	{#each pullHistory as pull, i}
		...
		<g opacity="0.8">
			<line x1={hStartX} y1={y} x2={hEndX} y2={y} stroke={color} stroke-width="6" stroke-linecap="round" stroke-dasharray="1 1" />
	...
	{#if !config?.pulls || activePullIndex < config.pulls}
		...
		<line x1={startX} y1={currentY} x2={endX} y2={currentY} stroke="#f3f4f6" stroke-width="12" stroke-linecap="round" />
		<line x1={startDragX} y1={currentY} x2={sliderX} y2={currentY} stroke={currentColor} stroke-width="6" stroke-linecap="round" stroke-dasharray="1 1" />
