<script lang="ts">
	...
	// 前面 (front face)
	const FX = 70,  FY = 190, FW = 110, FH = 100;
	...
	// ── 水位計算 ─────────────────────────────────────────────
	// 前面水面 Y（從底部往上長）
	function wfy(): number { return FB - (dropCount / 20) * FH; }
	// 後面水面 Y（套用深度向量）
	function wby(): number { return wfy() - DY; }
	...
	function animateDrop(fromCx: number, fromCy: number, toCx: number, toCy: number): Promise<void> {
		return new Promise(resolve => {
			dropCx = fromCx; dropCy = fromCy; dropVisible = true;
...
<div class="flex flex-col items-center gap-2 p-4 w-full select-none">
	...
	>
		<!-- ── 滴管（可點擊） ── -->
		<g onclick={handleClick} class="cursor-pointer" role="button" tabindex="0" aria-label="點擊開始滴水">
			<!-- 橡皮球 -->
			<ellipse cx={DROP_CX} cy="48" rx="20" ry="18"
				fill={started ? '#94a3b8' : '#f97316'}
				class="transition-colors duration-300"
			/>
			<!-- 玻璃管 -->
			<rect x={DROP_CX - 7} y="64" width="14" height="58"
				fill="white" stroke="#94a3b8" stroke-width="1.5" rx="3"
			/>
			<!-- 管內液體 -->
			<rect x={DROP_CX - 5} y="66" width="10" height="54"
				fill="#bfdbfe" rx="2"
			/>
			<!-- 尖端 -->
			<polygon points="{DROP_CX-6},{122} {DROP_CX+6},{122} {DROP_CX+2},{148} {DROP_CX-2},{148}"
		...
		<!-- ── 水滴動畫 ── -->
		{#if dropVisible}
			<ellipse cx={dropCx} cy={dropCy} rx={DROP_R} ry={DROP_R * 1.3}
		...
		{#if dropCount > 0}
			...
			<polygon
				...
				fill="#2563eb" fill-opacity="0.45"
			/>
			<!-- 水：頂面（較亮，受光） -->
			<polygon
				points="{FX},{wfy()} {FX+FW},{wfy()} {FX+FW+DX},{wby()} {FX+DX},{wby()}"
				fill="#93c5fd" fill-opacity="0.75"
			/>
		{/if}
		...
		<!-- 前面外框 -->
		<rect x={FX} y={FY} width={FW} height={FH}
			fill="none" stroke="#6b8aaa" stroke-width="2"
		/>
		<!-- 頂面外框 -->
		<polygon
			points="{FX},{FY} {FX+FW},{FY} {FX+FW+DX},{FY-DY} {FX+DX},{FY-DY}"
		...
		<text x={FX + FW/2} y={FB+22} text-anchor="middle" font-size="10" fill="#475569" font-weight="bold">1 cm</text>
		...
		<text
			...
			transform="rotate(-90, {FX-24}, {(FY+FB)/2})"
		>1 cm</text>
		...
		<line
			x1={FX+FW+4}    y1={FY+5}
			x2={FX+FW+DX+4} y2={FY-DY+5}
			stroke="#94a3b8" stroke-width="1"
		/>
		<line x1={FX+FW}    y1={FY+1} x2={FX+FW+6}    y2={FY+8}  stroke="#94a3b8" stroke-width="1.5" />
		<line x1={FX+FW+DX} y1={FY-DY+1} x2={FX+FW+DX+6} y2={FY-DY+8} stroke="#94a3b8" stroke-width="1.5" />
		<text x={depMidX+10} y={depMidY+14} text-anchor="middle" font-size="10" fill="#475569" font-weight="bold">1 cm</text>

		<!-- ── 計數器 ── -->
		{#if started && !done}
			<text x="285" y="25" text-anchor="end"
				font-size="17" font-weight="black" fill="#3b82f6"
			>{dropCount}<tspan font-size="11" fill="#64748b"> / 20</tspan></text>
		{/if}

		<!-- ── 完成標示 ── -->
		{#if done}
			<rect x={FX} y={FB + 28} width={FW + DX} height="34" rx="7"
				fill="#dcfce7" stroke="#4ade80" stroke-width="1.5"
			/>
			<text x={FX + (FW + DX)/2} y={FB + 50} text-anchor="middle"
				font-size="13" font-weight="bold" fill="#16a34a"
			>= 1 ml = 1 c.c. ✓</text>
		{/if}
	</svg>
</div>
