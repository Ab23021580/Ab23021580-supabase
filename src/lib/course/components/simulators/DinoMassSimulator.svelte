...
<div class="flex flex-col items-center gap-1 p-3 w-full select-none">
	...
	<svg width="100%" viewBox="0 0 340 240"
		preserveAspectRatio="xMidYMid meet" class="max-w-[360px] cursor-pointer"
		role="img" aria-label="質量不隨環境改變"
		onclick={handleClick}>
		...
		{:else if scene === 'water'}
			...
			<!-- 氣泡 -->
			{#each [60, 120, 200, 270, 310] as bx, i}
		...
		<!-- 質量標籤（始終顯示 30 塊） -->
		{#if scene}
			<rect x="100" y="200" width="140" height="28" rx="6"
				fill="white" fill-opacity="0.9" stroke="#94a3b8" stroke-width="1"/>
			<text x="170" y="219" text-anchor="middle"
				font-size="12" font-weight="bold" fill="#475569">
				質量 = 30 塊積木
			</text>
		{/if}
		...
		<!-- 未開始提示 -->
		{#if !scene}
			<text x="170" y="220" text-anchor="middle"
				font-size="12" fill="#94a3b8">點擊觀察</text>
		{/if}

		<!-- 完成標示 -->
		{#if done}
			<rect x="50" y="200" width="240" height="30" rx="7"
