<script lang="ts">
	...
	function rippleY(x: number, nowMs: number): number {
		...
		if (rt > 2.5) { rippleOn = false; return 0; }
		const dx     = x - RIPPLE_X;
		const decay  = Math.exp(-rt * 1.6);
		const spread = 10 + rt * 80;
		return 16 * decay
		     * Math.exp(-(dx * dx) / (spread * spread))
		     * Math.sin(0.5 * dx - 12 * rt);
	}

	// ── SVG path 產生器 ──────────────────────────────────────
	const XL = 254, XR = 346;   // 玻璃杯內壁 X 範圍
		...
		function loop(now: number) {
			...
			if (isDropped && !dropDone) {
				...
				if (e < PHASE1_MS) {
					...
					if (!rippleOn) { rippleOn = true; rippleT0 = now; waveStarted = true; }
...
<div class="flex flex-col gap-4 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden p-6 items-center">
	<p class="text-gray-600 font-bold text-center">點擊烏鴉讓牠鬆口投石，觀察水面的變化！</p>

	<div class="w-full max-w-[500px] h-[350px]">
		<svg
		...
		>
			...
			<!-- 實驗桌 -->
			<line x1="100" y1="350" x2="500" y2="350" stroke="#94a3b8" stroke-width="4" stroke-linecap="round" />
			...
			<!-- 玻璃杯（後畫蓋在水上，形成杯壁遮擋效果） -->
			<path d="M 250 150 L 250 350 L 350 350 L 350 150" fill="none" stroke="#cbd5e1" stroke-width="8" stroke-linejoin="round" />

			<!-- 石頭 -->
			<g transform="translate(300, {stoneY})">
				<path d="M -15 -10 L 10 -15 L 20 5 L 5 15 L -10 10 Z" fill="#64748b" stroke="#475569" stroke-width="2" />
			</g>
			...
			{#if isDropped && baseWaterY < 245}
				<g class="fade-in" style="animation-delay:0.3s; animation-fill-mode:both;">
					<line x1="220" y1="250" x2="380" y2="250" stroke="#ef4444" stroke-width="2" stroke-dasharray="4 2" />
					<line x1="370" y1="240" x2="370" y2={baseWaterY + 12} stroke="#ef4444" stroke-width="2" marker-end="url(#arrow)" />
