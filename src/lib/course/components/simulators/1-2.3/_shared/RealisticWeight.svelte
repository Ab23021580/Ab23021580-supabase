<script lang="ts">
	...
	// 顏色漸層設定
	const colors = $derived(material === 'brass' 
...
>
	<!-- 陰影 -->
	{#if !disabled}
		<ellipse cx="0" cy="0" rx={width/2 + 2} ry={width/6 + 1} fill="rgba(0,0,0,0.3)" filter="blur(2px)"/>
	{/if}

	<!-- 底部圓柱體主體 -->
	<!-- 圓柱下半部弧形 -->
	<path 
		d="M {-width/2} {-baseHeight} L {-width/2} 0 A {width/2} {width/6} 0 0 0 {width/2} 0 L {width/2} {-baseHeight} Z" 
		fill="url(#{gradientId})"
	...
	<!-- 頂部拿取圓鈕 (Knob) -->
	<path 
		d="M {-knobWidth/2} {-baseHeight} L {-knobWidth/3} {-totalHeight} A {knobWidth/3} {knobWidth/6} 0 0 1 {knobWidth/3} {-totalHeight} L {knobWidth/2} {-baseHeight} Z" 
		fill="url(#{gradientId})"
	/>
	<ellipse 
		cx="0" cy={-totalHeight} 
		rx={knobWidth/1.5} ry={knobWidth/3} 
		fill={colors.light}
	/>

	<!-- 質量標示 -->
	<text 
		x="0" y={-baseHeight/2 + 2} 
		text-anchor="middle" dominant-baseline="middle" 
		font-size={8 * scale} font-family="sans-serif" font-weight="bold" 
		fill={colors.dark} opacity="0.8"
	>
		{mass}g
	</text>
