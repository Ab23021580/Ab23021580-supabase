...
<div class="flex flex-col items-center gap-1 p-3 w-full select-none">
	...
	<svg width="100%" viewBox="0 0 370 255" preserveAspectRatio="xMidYMid meet"
		class="max-w-[390px] {step === 0 ? 'cursor-pointer' : ''}"
		onclick={handleClick} role="img" aria-label="歸零對比">
		...
		{#if leftDone}
			<rect x="15" y="195" width="150" height="22" rx="5" fill="#dcfce7" stroke="#4ade80" stroke-width="1.5"/>
		...
		<!-- 分界線 -->
		<line x1="185" y1="5" x2="185" y2="250" stroke="#e2e8f0" stroke-width="2" stroke-dasharray="5 4"/>
		...
		{#if rightDone}
			<rect x="205" y="195" width="150" height="22" rx="5" fill="#fee2e2" stroke="#fca5a5" stroke-width="1.5"/>
		...
		<!-- SOP 說明 -->
		{#if step >= 2}
			<rect x="10" y="225" width="350" height="24" rx="5"
