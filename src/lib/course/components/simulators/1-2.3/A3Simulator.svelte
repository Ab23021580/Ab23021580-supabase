...
<div class="flex flex-col items-center gap-1 p-3 w-full select-none">
	...
	<svg width="100%" viewBox="0 0 370 260" preserveAspectRatio="xMidYMid meet" class="max-w-[390px]">
		...
		<!-- 天平 -->
		<TopPanBalance cx={185} cy={120} tiltAngle={tiltAngle}/>

		{#if rightTotal > 0}
			<text x="240" y="70" text-anchor="middle" font-size="11" font-weight="bold" fill="#1e293b">{rightTotal}g</text>
		{/if}
		<text x="130" y="70" text-anchor="middle" font-size="11" font-weight="bold" fill="#1e293b">物體</text>
		...
		<!-- 砝碼區 -->
		<text x="185" y="178" text-anchor="middle" font-size="10" fill="#64748b">真實砝碼組（點擊放入托盤）</text>
		...
		<!-- 狀態訊息 -->
		<rect x="20" y="238" width="330" height="18" rx="4"
			fill={done ? '#dcfce7' : '#fef3c7'}
			stroke={done ? '#4ade80' : '#fbbf24'} stroke-width="1.5"/>
		<text x="185" y="250" text-anchor="middle" font-size="10"
			font-weight="bold" fill={done ? '#16a34a' : '#92400e'}>
