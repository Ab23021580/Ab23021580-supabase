...
<div class="flex flex-col items-center gap-1 p-3 w-full select-none">
	...
	<svg width="100%" viewBox="0 0 370 260" preserveAspectRatio="xMidYMid meet"
		class="max-w-[390px] {step === 0 ? 'cursor-pointer' : ''}"
		onclick={handleClick} role="img" aria-label="砝碼加總讀數">
		...
		<text x="130" y="70" text-anchor="middle" font-size="11" font-weight="bold" fill="#1e293b">飲料罐</text>
		<text x="240" y="70" text-anchor="middle" font-size="11" font-weight="bold" fill="#1e293b">{step >= 1 ? `${runningSum}g` : '?'}</text>

		<!-- 左盤飲料罐 (大致放在左盤上方) -->
		<rect x="85" y="45" width="20" height="28" rx="4"
		...
		{#if step >= 1}
			...
			<text x="318" y="162" font-size="14" font-weight="bold"
				fill={step >= 2 ? '#16a34a' : '#f97316'}>{runningSum}g</text>
		{/if}

		<!-- 揭曉飲料罐 -->
		{#if step >= 2}
			<rect x="20" y="178" width="330" height="68" rx="8"
				fill="#dcfce7" stroke="#4ade80" stroke-width="2"/>
			<text x="185" y="200" text-anchor="middle" font-size="13"
				font-weight="bold" fill="#16a34a">物體質量 = {TOTAL}g</text>
		...
		{:else if step === 0}
			<text x="185" y="200" text-anchor="middle" font-size="11" fill="#94a3b8">
				點擊開始加總
			</text>
		{/if}
	</svg>
