<script lang="ts">
	...
	const PART_NAMES: Record<Part, string> = {
		needle: '指針', screw: '校準螺絲', leftPan: '左盤', rightPan: '右盤'
	};
...
<div class="flex flex-col items-center gap-1 p-3 w-full select-none">
	...
	<svg width="100%" viewBox="0 0 370 250" preserveAspectRatio="xMidYMid meet" class="max-w-[390px]">
		...
		<!-- 天平共用元件 -->
		<TopPanBalance cx={185} cy={145} tiltAngle={0} highlightPart={currentHighlight} showLabels={done}/>

		<!-- 透明點擊層（疊在 Balance 上方） -->
		{#each CLICK_ZONES as zone}
			<rect
				x={zone.x} y={zone.y} width={zone.w} height={zone.h}
				fill="transparent" stroke={foundParts.has(zone.part) ? '#4ade80' : 'transparent'}
				stroke-width="2" rx="4" style="cursor:pointer"
				onclick={() => clickZone(zone.part)}/>
			<!-- 已找到的打勾標示 -->
			{#if foundParts.has(zone.part)}
				<circle cx={zone.x + zone.w - 8} cy={zone.y + 8} r="8" fill="#4ade80"/>
				<text x={zone.x + zone.w - 8} y={zone.y + 12} text-anchor="middle"
					font-size="10" fill="white">✓</text>
			{/if}
		{/each}

		<!-- 下方部位清單 -->
		<rect x="10" y="222" width="350" height="22" rx="5"
			fill={done ? '#dcfce7' : '#f1f5f9'} stroke={done ? '#4ade80' : '#e2e8f0'} stroke-width="1.5"/>
		<text x="185" y="237" text-anchor="middle" font-size="10"
			fill={done ? '#16a34a' : '#64748b'}>
			{#if done}
				四個部位全部找到！
			{:else}
				{PARTS.filter(p => foundParts.has(p)).map(p => PART_NAMES[p]).join('、') || '點擊天平上的各部位'}
			{/if}
		</text>
	</svg>
</div>
