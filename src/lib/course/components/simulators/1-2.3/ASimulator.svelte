<script lang="ts">
	...
	let { courseState, onCorrect }: {
		courseState: CourseState; onCorrect: () => void;
	} = $props();
	...
	<svg width="100%" viewBox="0 0 360 230" preserveAspectRatio="xMidYMid meet" class="max-w-[380px]">
		...
		<g onclick={() => reveal(0)} style="cursor:pointer">
			...
			{:else}
				...
				<line x1="25" y1="90" x2="110" y2="90" stroke="#64748b" stroke-width="5" stroke-linecap="round"/>
			...
			{/if}
			<rect x="15" y="30" width="100" height="140" rx="8"
				fill="none" stroke={TOOLS[0].revealed ? '#4ade80' : '#475569'} stroke-width="2"/>
			<text x="65" y="185" text-anchor="middle" font-size="11" font-weight="bold"
		...
		<g onclick={() => reveal(1)} style="cursor:pointer">
			...
			{:else}
				<!-- 等臂天平（兩盤懸掛更低，更對稱） -->
				<line x1="145" y1="95" x2="215" y2="95" stroke="#64748b" stroke-width="5" stroke-linecap="round"/>
			...
			{/if}
			<rect x="130" y="30" width="100" height="140" rx="8"
				fill="none" stroke={TOOLS[1].revealed ? '#4ade80' : '#475569'} stroke-width="2"/>
			<text x="180" y="185" text-anchor="middle" font-size="11" font-weight="bold"
		...
		<g onclick={() => reveal(2)} style="cursor:pointer">
			...
			{:else}
				<rect x="260" y="135" width="70" height="28" rx="5" fill="#334155" stroke="#475569" stroke-width="1.5"/>
				<rect x="264" y="115" width="62" height="22" rx="3" fill="#0f172a"/>
				<text x="295" y="130" text-anchor="middle" font-size="12" font-family="monospace" fill="#4ade80">0.0 g</text>
				<rect x="255" y="105" width="80" height="12" rx="3" fill="#94a3b8"/>
				<rect x="270" y="160" width="50" height="6" rx="2" fill="#64748b"/>
			{/if}
			<rect x="245" y="30" width="100" height="140" rx="8"
				fill="none" stroke={TOOLS[2].revealed ? '#4ade80' : '#475569'} stroke-width="2"/>
			<text x="295" y="185" text-anchor="middle" font-size="11" font-weight="bold"
				fill={TOOLS[2].revealed ? '#16a34a' : '#94a3b8'}>{TOOLS[2].label}</text>
		</g>

		{#if done}
			<rect x="30" y="200" width="300" height="22" rx="5" fill="#dcfce7" stroke="#4ade80" stroke-width="1.5"/>
