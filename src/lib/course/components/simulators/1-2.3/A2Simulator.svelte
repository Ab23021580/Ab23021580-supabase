<script lang="ts">
	...
	const sideHint = $derived(
		zeroOffset > 2 ? '右盤較高 → 朝右盤方向旋入螺絲' :
		zeroOffset < -2 ? '左盤較高 → 朝左盤方向旋入螺絲' :
		'指針置中 ✓ 歸零完成'
	);
</script>
...
<div class="flex flex-col items-center gap-1 p-3 w-full select-none">
	...
	<svg width="100%" viewBox="0 0 370 250" preserveAspectRatio="xMidYMid meet" class="max-w-[390px]">
		...
		<!-- 天平（初始向右傾斜） -->
		<TopPanBalance cx={185} cy={145} tiltAngle={0} {zeroOffset}
			highlightPart={!done ? 'screw' : null}/>

		<!-- 螺絲點擊熱區 (右側校準螺絲) -->
		{#if !done}
			<circle cx={313} cy={145} r="15" fill="transparent"
				stroke="#f97316" stroke-width="2" stroke-dasharray="4 3"
				style="cursor:pointer" onclick={turnScrew}/>
			<text x="313" y="170" text-anchor="middle" font-size="9" fill="#f97316">
				點擊旋轉
			</text>
		{/if}
		...
		<!-- 狀態提示 -->
		<rect x="20" y="222" width="330" height="22" rx="5"
