	...
	<div class="relative w-full aspect-video bg-slate-100">
		<svg viewBox="0 0 400 300" class="absolute inset-0 w-full h-full select-none">
			...
			<g transform="translate(0, 30)">
				<ellipse cx={cx} cy={cy} rx="100" ry="20" fill="rgba(0,0,0,0.15)" filter="url(#shadow2)"/>
				<!-- Base -->
				<path d="M {cx - 90} {cy - 20} L {cx + 90} {cy - 20} Q {cx + 100} {cy - 20} {cx + 100} {cy} L {cx + 100} {cy + 5} Q {cx + 100} {cy + 15} {cx + 90} {cy + 15} L {cx - 90} {cy + 15} Q {cx - 100} {cy + 15} {cx - 100} {cy + 5} L {cx - 100} {cy} Q {cx - 100} {cy - 20} {cx - 90} {cy - 20} Z" fill="url(#metal-base2)" stroke="#475569" stroke-width="1.5"/>
				
				<!-- Pan -->
				<ellipse cx={cx} cy={cy - 25} rx="70" ry="14" fill="url(#metal-pan2)" stroke="#64748b" stroke-width="2"/>
			...
			<g transform="translate({cx}, {cy - 5})">
				...
				<g style="transition: all 0.5s cubic-bezier(0.4, 0, 0.2, 1); transform: scale(1, {currentLvl.scaleY})">
					{#if force === 0}
						<!-- Flat paper -->
						<rect x="-80" y="-10" width="160" height="10" rx="2" fill="url(#tissue-grad)" filter="url(#tissue-shadow)" stroke="#cbd5e1" stroke-width="1"/>
					{:else}
						<!-- Crumpled ball, simulated using layered circles to look wrinkly -->
						<circle cx="0" cy={-currentLvl.r + 5} r={currentLvl.r} fill="url(#tissue-grad)" filter="url(#tissue-shadow)" stroke="#cbd5e1" stroke-width="1.5"/>
						<path d="M {-currentLvl.r * 0.5} {-currentLvl.r} Q 0 {-currentLvl.r*1.5} {currentLvl.r * 0.6} {-currentLvl.r*0.8}" fill="none" stroke="#94a3b8" stroke-width="2" stroke-linecap="round"/>
						<path d="M {-currentLvl.r * 0.7} {-currentLvl.r*0.5} Q {-currentLvl.r*0.2} 0 {currentLvl.r * 0.4} {-currentLvl.r*0.3}" fill="none" stroke="#cbd5e1" stroke-width="2" stroke-linecap="round"/>
					{/if}
				</g>
			</g>
			...
			{#if force > 0 && force < 3}
				<path d="M {cx} 50 L {cx-20} 80 L {cx-10} 80 L {cx-10} 120 L {cx+10} 120 L {cx+10} 80 L {cx+20} 80 Z" fill="#ef4444" class="animate-bounce" opacity="0.6"/>
			{/if}
		</svg>
	</div>
