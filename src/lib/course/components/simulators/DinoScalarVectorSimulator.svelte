<script lang="ts">
	...
	// 左右半場座標
	const MID = 185;
	const SPACE_CX = 92, EARTH_CX = 278;
...
<div class="flex flex-col items-center gap-1 p-3 w-full select-none">
	...
	<svg width="100%" viewBox="0 0 370 280"
		...
		{/each}
		<!-- 左側標題 -->
		<text x={SPACE_CX} y="268" text-anchor="middle" font-size="14" font-weight="bold" fill="#94a3b8">🌌 太空</text>

		<!-- 恐龍 (漂浮) -->
		<LegoDino x={SPACE_CX} y={DINO_Y} scale={0.7} builtCount={30} floating={true} />

		<!-- ══ 分界線 ══ -->
		<line x1={MID} y1="0" x2={MID} y2="280" stroke="#64748b" stroke-width="2" stroke-dasharray="6 4" />
		...
		<!-- 右側標題 -->
		<text x={EARTH_CX} y="268" text-anchor="middle" font-size="14" font-weight="bold" fill="#3b82f6">🌍 地球</text>
		...
		{#if started}
			<!-- 左側：純量 (無方向) -->
			<text x={SPACE_CX} y="40" text-anchor="middle" font-size="12" font-weight="bold" fill="#f8fafc">純量：300 克</text>
			
			<!-- 右側：向量 (向下) -->
			<text x={EARTH_CX} y="40" text-anchor="middle" font-size="12" font-weight="bold" fill="#1e40af">向量：向下 300 克重</text>
