...
<div class="flex flex-col items-center gap-1 p-3 w-full select-none">
	...
	<svg width="100%" viewBox="0 0 380 265"
		preserveAspectRatio="xMidYMid meet" class="max-w-[400px] cursor-pointer"
		onclick={handleClick} role="img" aria-label="1mg一粒鹽放大">

		<!-- ── 左半：放大鏡 ── -->
		<rect x="0" y="0" width="195" height="265" rx="8" fill="#f8fafc"/>

		<!-- 放大鏡外框 -->
		<circle cx="90" cy="120" r="68"
			fill="white" fill-opacity="0.9"
			stroke="#94a3b8" stroke-width="3"/>
		<circle cx="90" cy="120" r="65"
			fill={step >= 1 ? '#f0f9ff' : '#f8fafc'}
			stroke="none"/>
		<!-- 手柄 -->
		<line x1="140" y1="170" x2="170" y2="200"
			stroke="#94a3b8" stroke-width="8" stroke-linecap="round"/>

		<!-- 未放大時：只有小白點（代表看不清的鹽粒） -->
		{#if step === 0}
			<circle cx="90" cy="120" r="3" fill="#94a3b8" opacity="0.6"/>
			<text x="90" y="200" text-anchor="middle" font-size="10" fill="#cbd5e1">
				點擊放大觀察
			</text>
		{/if}
		...
		{#if step >= 2}
			...
			{#each [0,1,2] as row}
				{#each [0,1,2] as col}
					...
					<circle cx={58 + col * 32} cy={92 + row * 32} r="12"
						fill={isNa ? '#93c5fd' : '#fca5a5'}
						stroke={isNa ? '#1d4ed8' : '#b91c1c'}
						stroke-width="1.5"/>
					<text x={58 + col * 32} y={92 + row * 32 + 4}
		...
		<!-- ── 分界線 ── -->
		<line x1="200" y1="10" x2="200" y2="255"
		...
		<!-- 防風罩 -->
		{#if step >= 2}
			<rect x="222" y="40" width="140" height="120" rx="6"
				fill="#f1f5f9" fill-opacity="0.5"
				stroke="#cbd5e1" stroke-width="1.5"
				stroke-dasharray="4 3"/>
			<text x="292" y="58" text-anchor="middle" font-size="9" fill="#94a3b8">
				防風罩
			</text>
		{/if}
		...
		<!-- 秤盤上的鹽粒 -->
		{#if step >= 2}
			<circle cx="292" cy="130" r="4"
				fill="#e2e8f0" stroke="#94a3b8" stroke-width="1"/>
			<text x="292" y="108" text-anchor="middle" font-size="9" fill="#64748b">
				一粒鹽
			</text>
		{/if}

		<!-- 電子秤本體（使用共用元件） -->
		<DigitalScale x={292} y={210}
			reading={step >= 2 ? '0.001 g' : '---'}
			scale={1.1}/>

		<!-- 指數換算 -->
		{#if step >= 2}
			<text x="292" y="242" text-anchor="middle" font-size="10"
				font-weight="bold" fill="#0369a1">
				= 10⁻³ g = 1 mg
			</text>
		{/if}
		...
		<!-- 完成結論 -->
		{#if step >= 3}
			<rect x="10" y="242" width="360" height="19" rx="5"
				fill="#dcfce7" stroke="#4ade80" stroke-width="1.5"/>
			<text x="190" y="255" text-anchor="middle"
				font-size="11" font-weight="bold" fill="#16a34a">
				1mg = 10⁻³g：比公克小1000倍
			</text>
		{/if}
	</svg>
