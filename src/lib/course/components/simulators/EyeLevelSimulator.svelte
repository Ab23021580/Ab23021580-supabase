<script lang="ts">
	...
	// 量筒參數
	const CYL_X = 220, CYL_Y = 260, CYL_H = 210;
	const WATER = 0.6;
...
<div class="flex flex-col items-center gap-1 p-3 w-full select-none">
	...
	<svg width="100%" viewBox="0 0 340 300"
		preserveAspectRatio="xMidYMid meet" class="max-w-[360px]"
		role="img" aria-label="平視法讀取量筒">
		...
		{#each EYES as eye}
			...
			<!-- 連線（點擊後才顯示） -->
			{#if isSelected}
				<line x1={eyeX + 30} y1={eye.y}
					x2={CYL_X - 22} y2={WATER_Y}
					stroke={eye.correct ? '#22c55e' : '#ef4444'}
					stroke-width="1.5"
					stroke-dasharray={eye.correct ? 'none' : '5 3'}
				/>
				<!-- 正確/錯誤標記 -->
				<text x={(eyeX + 30 + CYL_X - 22) / 2} y={(eye.y + WATER_Y) / 2 - 6}
			...
			<!-- 眼睛圖示（可點擊） -->
			<g class="cursor-pointer" onclick={() => handleSelect(eye)}
				role="button" tabindex="0" aria-label={eye.label}>
				<!-- 眼睛外框 -->
				<ellipse cx={eyeX} cy={eye.y} rx="26" ry="16"
					fill={isSelected ? (eye.correct ? '#dcfce7' : '#fef2f2') : '#f1f5f9'}
					stroke={isSelected ? (eye.correct ? '#22c55e' : '#ef4444') : '#64748b'}
					stroke-width="2" />
				<!-- 瞳孔 -->
				<circle cx={eyeX + 6} cy={eye.y} r="7"
					fill="#1e293b" />
				<circle cx={eyeX + 8} cy={eye.y - 2} r="2.5"
					fill="white" />

				<!-- 標籤 -->
				<text x={eyeX - 35} y={eye.y + 4}
		...
		<!-- 底部說明 -->
		{#if done}
			<rect x="30" y="272" width="280" height="28" rx="6"
				fill="#dcfce7" stroke="#4ade80" stroke-width="1.5"/>
			<text x="170" y="291" text-anchor="middle"
