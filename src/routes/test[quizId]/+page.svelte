<script lang="ts">
	...
	let mergedQuizIds = $state<string[]>([]);
	...
	async function startMergedQuiz(matchedQuizzes: ApiQuiz[], prefix: string) {
		...
		isStarting = true;
		mergedQuizIds = matchedQuizzes.map((q) => q.id);
		...
		for (const mq of matchedQuizzes) {
			try {
				...
				mq.questions.forEach((q) => { q.userAnswers = []; });
			} catch (err) {
		...
		try {
			learningSessionId = await startLearningSession(`/test${page.params.quizId || '1-1.1'}`, matchedQuizzes[0].id);
		} finally {
			isStarting = false;
		}
	}
